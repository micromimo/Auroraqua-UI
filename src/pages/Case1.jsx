...
import { Wifi, WifiOff, Settings, Info, Home, Activity } from 'lucide-react'
import Sidebar from '../components/case1/Case1Sidebar'
...
export default function Case1() {
  ...
  return (
    <div className="w-screen h-screen relative overflow-hidden" style={{
        ...
        <div className="absolute top-0 left-0 right-0 flex items-start justify-between px-6 py-4 z-40">
          <div className="flex items-center gap-3">
            <div className="glass rounded-2xl px-4 py-2 flex items-center gap-3">
              ...
              <Link to="/settings" className="flex items-center gap-2 text-sm text-body hover:text-heading transition-colors">
                <Settings className="w-4 h-4" />
                <span>设置</span>
              </Link>
              <Link to="/about" className="flex items-center gap-2 text-sm text-body hover:text-heading transition-colors">
                <Info className="w-4 h-4" />
                <span>关于</span>
              </Link>
              <div className="h-6 w-px bg-gray-300/50" />
              <Link to="/init" className="flex items-center gap-2 text-sm text-body hover:text-heading transition-colors">
      ...
      <div className="fixed bottom-4 right-4 glass-shortcut rounded-xl px-3 py-2 text-xs text-body">
        <span>快捷键: </span>
        <kbd className="px-2.5 py-1 bg-white/60 rounded-lg text-body ml-1.5">F</kbd>
        <kbd className="px-2.5 py-1 bg-white/60 rounded-lg text-body ml-1.5">←→</kbd>
        <kbd className="px-2.5 py-1 bg-white/60 rounded-lg text-body ml-1.5">M</kbd>
      </div>
    </div>
  )
}