...
`);
  ...
  const renderMarkdown = (mdContent) => {
    ...
    const renderInline = (text) => {
      ...
      const patterns = [
        { regex: /^~~([^~]+)~~/, component: (match) => <s key={key++} className="line-through text-slate-500">{match[1]}</s> },
        { regex: /^\*\*([^*]+)\*\*/, component: (match) => <strong key={key++} className="font-bold text-slate-800">{match[1]}</strong> },
        { regex: /^\*([^*]+)\*/, component: (match) => <em key={key++} className="italic text-slate-700">{match[1]}</em> },
        { regex: /^`([^`]+)`/, component: (match) => <code key={key++} className="bg-slate-200/50 px-1.5 py-0.5 rounded text-xs font-mono text-pink-600">{match[1]}</code> },
        { regex: /^\[([^\]]+)\]\(([^)]+)\)/, component: (match) => <a key={key++} href={match[2]} target="_blank" rel="noopener noreferrer" className="text-pink-600 hover:text-pink-700 hover:underline">{match[1]}</a> },
      ];
    ...
    const parseList = (startIndex) => {
        ...
        if (isCheckboxList && (line.startsWith('- [x]') || line.startsWith('- [ ]') || line.startsWith('- [ ]'))) {
          ...
          listItems.push(
            <li key={j} className="flex items-center gap-2">
              ...
              <span className={`text-sm ${checked ? 'text-slate-400 line-through' : 'text-slate-600'}`}>{renderInline(text)}</span>
            </li>
          );
          j++;
          ...
          listItems.push(
            <li key={j} className="flex items-start gap-2">
              ...
              <span className="text-sm text-slate-600">{renderInline(text)}</span>
            </li>
          );
          j++;
          ...
          listItems.push(
            <li key={j} className={`flex items-start gap-2 ${indent > 0 ? 'ml-6' : ''}`}>
      ...
      elements.push(
        <ListComponent key={startIndex} className={`list-none space-y-${isCheckboxList ? '2' : '1'} my-3`}>
          {listItems}
        </ListComponent>
      );
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('---') || line.startsWith('***')) {
        elements.push(<hr key={i} className="my-6 border-t border-white/30" />);
        i++;
        continue;
      }

      if (line.startsWith('# ')) {
        elements.push(<h1 key={i} className="text-2xl font-bold text-slate-800 mt-8 mb-4 pb-2 border-b border-white/20">{renderInline(line.slice(2))}</h1>);
        i++;
        continue;
      }

      if (line.startsWith('## ')) {
        elements.push(<h2 key={i} className="text-xl font-bold text-slate-700 mt-6 mb-3">{renderInline(line.slice(3))}</h2>);
        i++;
        continue;
      }

      if (line.startsWith('### ')) {
        elements.push(<h3 key={i} className="text-lg font-bold text-slate-600 mt-5 mb-2">{renderInline(line.slice(4))}</h3>);
        i++;
        continue;
      }
      ...
      if (line.startsWith('> ')) {
        ...
        elements.push(<blockquote key={i} className="border-l-4 border-pink-400 pl-4 my-4 italic text-slate-500 bg-pink-500/5 rounded-r-xl py-2 pr-4">{renderInline(quoteContent)}</blockquote>);
        continue;
      }
      ...
      if (line.startsWith('```')) {
        ...
        if (lang === 'mermaid') {
          ...
          elements.push(
            <div key={i} className="my-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-500 font-medium">mermaid</span>
                <button
                  onClick={() => {
                    setEditingMermaidCode(code.trim());
                    setMermaidCodeIndex(i);
          ...
          elements.push(
            <div key={i} className="my-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-500 font-medium">{lang || 'code'}</span>
              </div>
              <pre className="bg-black/15 rounded-xl p-4 text-xs font-mono overflow-x-auto text-slate-700 whitespace-pre-wrap"><code>{code}</code></pre>
            </div>
          );
        }
        i++;
      ...
      if (line.startsWith('|')) {
        ...
        while (i < lines.length && lines[i].startsWith('|')) {
          ...
          if (lines[i + 1] && lines[i + 1].startsWith('|') && lines[i + 1].includes('---')) {
            tableRows.push(
              <tr key={i}>
                {cells.map((cell, idx) => <th key={idx} className="border-b-2 border-pink-300/50 px-4 py-2 text-xs font-semibold text-slate-600 bg-white/20">{cell.trim()}</th>)}
              </tr>
            );
            i += 2;
          } else {
            tableRows.push(
              <tr key={i} className="hover:bg-white/10 transition-colors">
                {cells.map((cell, idx) => <td key={idx} className="border-b border-white/10 px-4 py-2 text-xs text-slate-600">{cell.trim()}</td>)}
              </tr>
            );
            i++;
          }
        }
        elements.push(<table key={i} className="w-full rounded-xl overflow-hidden liquid-glass my-4">{tableRows}</table>);
        continue;
      }
      ...
      if (line.trim()) {
        elements.push(<p key={i} className="text-sm text-slate-600 my-3 leading-relaxed">{renderInline(line)}</p>);
      }
      i++;
    }
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-700">Markdown 文章</h2>
        ...
        <div className="max-w-3xl mx-auto">
          {isEditing ? (
            <textarea
              ...
              className="w-full h-full min-h-[500px] p-4 bg-white/20 rounded-xl border border-white/30 text-sm font-mono text-slate-700 resize-none focus:outline-none focus:border-pink-400/50 focus:bg-white/30 transition-all"
          ...
          >
            ...
            >
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-sm text-slate-600 text-center">链接已复制到剪贴板</p>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                ...
                <button
                  ...
                  className="glass-button text-slate-500 hover:text-slate-700 p-2"
                >
              ...
              <div className="flex flex-col gap-4 h-[calc(80vh-100px)]">
                <div className="flex-1 min-h-0">
                  ...
                  <textarea
                    ...
                    className="w-full h-[calc(50%-1rem)] p-4 bg-white/30 rounded-xl border border-white/40 text-sm font-mono text-slate-700 resize-none focus:outline-none focus:border-pink-400/50 focus:bg-white/40 transition-all"
